...
(defproject ttt-be "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "ttt-be.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.6.0"]]}
             :uberjar {:aot [ttt-be.server]}}
  :main ^{:skip-aot true} ttt-be.server)
